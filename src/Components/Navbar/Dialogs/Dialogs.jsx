...
function Dialogs(props) {
  let dialogElemets = props.state.dialogs.map((d) => (
    <DialogItems id={d.id} key={d.id} name={d.name} />
  ));

  let messageElements = props.state.messages.map((m) => (
    <MessageItems key={m.id} message={m.message} />
  ));
  ...
  return (
    <div className={clascss.dialogs}>
      <div className={clascss.dialogItems}>{dialogElemets}</div>
